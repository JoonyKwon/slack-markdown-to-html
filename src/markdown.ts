...
const replaceInWindows = (
    ...
    if (!currentWindow.replacementDisabled && openingMatch && openingMatch.index <= currentWindow.end) {
        ...
        if (closingMatch && closingMatch.index + closingDelimiterLength - 1 <= currentWindow.end) {
            ...
            const openingReplacementString = `${replacementOpeningLiteral}${
                (!noAlphaNumericPadded && openingMatch.openingCapturedWhitespace) || ''
            }`;
            const closingReplacementString = `${
                (!noAlphaNumericPadded && closingMatch.closingCapturedWhitespace) || ''
            }${asymmetric ? closingMatch[0] : ''}${replacementClosingLiteral}`;
